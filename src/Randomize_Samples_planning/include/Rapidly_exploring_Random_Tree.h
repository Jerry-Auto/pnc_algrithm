...
public:
    RRT( double robotRadius, double expandDis,double goalSampleRate,int maxIter);

    void init_solver(WorldMap* map);

    pair<vector<double>,vector<double>>planning(bool plot=false);
...
private:
    ...
    Node* begin=nullptr;//根节点
    Node* end=nullptr;//终节点
