...
void text(Numeric x, Numeric y, const std::string& s = "") {
    ...
    PyObject *args = nullptr, *kwargs = nullptr, *res = nullptr;
    
    try {
        // 创建参数元组
        args = PyTuple_New(2);
        if (!args) throw std::runtime_error("Failed to create Python tuple");
        
        PyTuple_SetItem(args, 0, PyFloat_FromDouble(static_cast<double>(x)));
        PyTuple_SetItem(args, 1, PyFloat_FromDouble(static_cast<double>(y)));
        
        // 创建关键字参数字典
        kwargs = PyDict_New();
        if (!kwargs) throw std::runtime_error("Failed to create Python dict");
        
        PyDict_SetItemString(kwargs, "s", PyUnicode_FromString(s.c_str()));
        
        // 添加字体安全参数
        PyDict_SetItemString(kwargs, "family", PyUnicode_FromString("sans-serif"));
        PyDict_SetItemString(kwargs, "fontsize", PyLong_FromLong(10));
        PyDict_SetItemString(kwargs, "ha", PyUnicode_FromString("center"));
        
        // 安全调用
        res = PyObject_Call(
            detail::_interpreter::get().s_python_function_text,
            args, kwargs);
            
        if (!res) {
            PyErr_Print();  // 打印Python错误但不抛出异常
            std::cerr << "Warning: Text rendering failed for: " << s << std::endl;
        }
