...
RRTConnect::RRTConnect( double robotRadius, double expandDis,double goalSampleRate, int maxIter) : robot_radius(robotRadius),
                                                             expand_dis(expandDis),
                                                             goal_sample_rate(goalSampleRate), max_iter(maxIter) {}

void RRTConnect::init_solver(WorldMap* map)
{
    Node* str=new Node(map->get_start().first,map->get_start().second);
    Node* en=new Node(map->get_goal().first,map->get_goal().second);
    this->begin = str;
    this->end=en;
    std::vector<WorldMap::Obstacle> obs=map->get_Obstacle();
    for(size_t i=0;i<obs.size();i++)
    {
        vector<double> obstacle={obs[i].x,obs[i].y,sqrt(pow(obs[i].height/2,2)+pow(obs[i].width/2,2))};
        obstacle_list.push_back(obstacle);
    }
    play_area=map->getBounds();
    if(play_area[0]>play_area[2])
    {
        rand_area.push_back(play_area[2]-1);
    }
    else{
        rand_area.push_back(play_area[0]-1);
    }

    if(play_area[1]>play_area[3])
    {
        rand_area.push_back(play_area[1]+1);
    }
    else{
        rand_area.push_back(play_area[3]+1);
    }
    map_=map;
}
 ...
 * @return 轨迹数据
 */
pair<vector<double>, vector<double>> RRTConnect::planning(bool plot) {
    ...
    for (int i = 0; i < max_iter; i++) {
            ...
            if (isInsidePlayArea(new_node_2) && obstacleFree(new_node_2)) {
                ...
                while (true) {
                    ...
                    if (abs(new_node_2->x - new_node->x) < 0.00001 && abs(new_node_2->y - new_node->y) < 0.00001) {
                        ...
                        if(plot){draw(rnd_node, new_node);plt::show();}
        ...
        //cout<<new_node->x<<","<<new_node->y<<endl;
        if(plot){draw(rnd_node, new_node);}
    }
    return {};
...
void RRTConnect::draw(Node *node1, Node *node2) {
    static bool is_first_call = true;  // 静态变量，仅第一次调用时初始化
    if (is_first_call) {
