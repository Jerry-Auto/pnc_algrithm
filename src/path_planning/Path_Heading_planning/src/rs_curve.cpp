...
std::vector<curve_type> RSCurve::GetRSCurve(double radius,
...
/// @param end_pos 
/// @return 
curve_type RSCurve::GetBestRSCurve(double radius,
        const point_type& start_pos, const point_type& end_pos)
{
    radius_=radius;
    start_pos_=start_pos;
    end_pos_=end_pos;
    auto all_curves =GetRSCurve(radius,start_pos, end_pos);
...
std::vector<point_type> RSCurve::GetRSPoint(const curve_type& rs_curve,double step_size) {
    ...
    for (unsigned int i = 0; i <= slice_num; i++) {
        double seg = i * step_size/radius_;//当前点距离起点的弧长
        double v;//
        ...
        for (unsigned int j = 0; j < 5u; j++) {
            if (rs_curve.second.first[j] < 0.0) {
                v = std::max(-seg, rs_curve.second.first[j]);//第j段的总长度
                seg += v;//倒车时总弧长减去j段的总长
            } else {
                v = std::min(seg, rs_curve.second.first[j]);//第j段的总长度
                seg -= v;//前进时总弧长减去j段的总长    
            }

            double phi = temp_point.second;//起始角度
            switch (rs_curve.first.first[j]) {
                case L:
                    temp_point.first.first += radius_*(sin(phi + v) - sin(phi));
                    temp_point.first.second += radius_*(-cos(phi + v) + cos(phi));
                    temp_point.second = NormalizeAngle(phi + v);
                    break;
                case R:
                    temp_point.first.first +=  radius_*(-sin(phi - v) + sin(phi));
                    temp_point.first.second +=  radius_*(cos(phi - v) - cos(phi));
                    ...
                    break;
                case S:
                    temp_point.first.first +=  radius_*v * cos(phi);
                    temp_point.first.second +=  radius_*v * sin(phi);
                    temp_point.second = NormalizeAngle(phi);
                    break;
                case N:
                    break;
            }
