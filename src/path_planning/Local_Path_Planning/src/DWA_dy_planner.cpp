...
DWADyPlanner::DWADyPlanner(const Config& config)
    : config_(config) {}

    void DWADyPlanner::read_map_data(WorldMap& map,ElectricVehicleDynamicsModel& car){
...
DWADyPlanner::planning_series() {
    ...
    while(true) {
        ...
        // 更新状态
        current_state = ElectricVehicleDynamicsModel::kinematics(best_v, best_yaw_rate,config_.dt,current_state);
...
DWADyPlanner::get_next_ctl_trj(ElectricVehicleDynamicsModel::VehicleState current_state) {
    ...
    for(double v = min_v; v <= max_v; v += config_.speed_resolution) {
        for(double yaw_rate = min_yaw_rate; yaw_rate <= max_yaw_rate; 
            ...
            double speed_cost = calc_speed_cost(vp) / sum_speed_cost;
            double obs_cost = 0;
            if(sum_obs_cost!=0){
...
DWADyPlanner::calc_dynamic_window(const ElectricVehicleDynamicsModel::VehicleState& state) const {
    // 方向盘动态窗口
    double v_min = std::max(config_.min_speed, state.steering_angle - config_.max_decel * config_.dt);
    double v_max = std::min(config_.max_speed, state.steering_angle + config_.max_accel * config_.dt);
    // 加速度动态窗口
    double yaw_rate_min = -config_.max_yaw_rate+0.5;
...
DWADyPlanner::predict_trajectory(double v, double yaw_rate,
    ...
    for(double t = config_.dt; t <= config_.predict_time; t += config_.dt) {
        state = ElectricVehicleDynamicsModel::kinematics(v, yaw_rate,config_.dt, state);
        x_pre.push_back(state.x);
        y_pre.push_back(state.y);
        yaw_pre.push_back(state.yaw);
    }   
...
double DWADyPlanner::calc_goal_cost(const std::vector<std::vector<double>>& trajectory,
    ...
    // double dnx=goal.first - trajectory[0][1];
    // double dny = goal.second - trajectory[1][1];
    return dx*dx+dy*dy;//+5*(dnx*dnx+dny*dny)
}

double DWADyPlanner::calc_speed_cost(double v) const {
    // 鼓励接近最大速度
    return config_.max_speed - v;
}

double DWADyPlanner::calc_obstacle_cost(
...
bool DWADyPlanner::check_collision(
    const std::vector<std::vector<double>>& trajectory,
    const std::vector<WorldMap::Obstacle>& obstacles) const 
{
...
/// @param goal 
/// @return 
double DWADyPlanner::calc_head_cost(const std::vector<std::vector<double>>& trajectory,
                               const std::pair<double, double>& goal) const {
...
void DWADyPlanner::plot_planning(WorldMap* map,ElectricVehicleDynamicsModel* car) {
    int k=3,i=0;
    double t=0;  
    double goal_threshold = std::pow(config_.robot_radius * 1.5, 2); // 目标点阈值  
    ...
    while(true) {
        // 获取下一步最佳控制指令和轨迹
        auto [best_v, best_yaw_rate, best_trajectory] = get_next_ctl_trj(current_state);
        std::cout<<"deta:"<<best_v<<" a:"<<best_yaw_rate<<" v:"<<current_state.vx<<std::endl;       
        // 绘制当前状态
